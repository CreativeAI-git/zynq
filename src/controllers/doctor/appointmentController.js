...
const RRule = rruleModule.RRule || rruleModule.default?.RRule || rruleModule.default;
    ...
    try {
        ...
        const result = await Promise?.all(appointments?.map(async (app) => {
            ...
            if (app.start_time && app.end_time) {
                ...
                videoCallOn = app.type === 'Video Call' && ['Scheduled', 'Rescheduled', 'Ongoing'].includes(app.status) && dayjs.utc().isAfter(startUTC) && dayjs.utc().isBefore(endUTC);
            }
